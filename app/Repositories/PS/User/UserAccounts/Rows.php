...
class Rows extends \App\Comptechsoft\Datatable\Rows
{
	...
	{
		$this->searcheableColumns([1 => 'id', 2 => 'categorie']);
		$this->orderableColumns([1 => 'id', 2 => 'categorie']);
		...
		$this->addCells([
			...
			// 		->withRecord($record)
			// 		->withUpdateRoute(\URL::route('superadmin.nomenclatoare.categorii-parteneri.get-action-form', ['action' => 'update', 'id' => $record->id]))
			// 		->withDeleteRoute(\URL::route('superadmin.nomenclatoare.categorii-parteneri.get-action-form', ['action' => 'delete', 'id' => $record->id]))
			// 		->render();
			// }),
		]);
	}
	...
	{
		return Categoriepartener::count();
	}
	...
	protected function filteredRecordsCount()
	{
		...
		return Categoriepartener::whereRaw($where)->count();
	}
	...
	protected function getDataSet()
	{
		...
		$result = 
			Categoriepartener::orderBy( $order_criteria['field'], $order_criteria['direction'] )
			;
		/*
		 * WhereRaw
		 */
