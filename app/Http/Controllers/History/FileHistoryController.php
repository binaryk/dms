...
class FileHistoryController extends BaseController
{
    ...
    {
        $this->makeNavigation();
        $dir = DirectorFiles::find($id);
        if(! $dir){
            return error('Nu exista director cu id:'.$id);
        }
        $grid = (new Grid())
            ->parameters( Input::all() )
            ->withParameter('file_id', $dir->id)
            ->setUrl();
        ...
        return
            view('director-files.grid.index')
                ...
                ->withToolbar( str_replace([Chr(13), Chr(10), Chr(9)], '', view('director-files.grid.toolbar')->withActionRoute(\URL::route('director-files.get-action-form', ['file_id' => $dir->id, 'action' => 'insert','grid_id' => $grid->getId()]))->render()) )
                ->withIcon(NULL)
    ...
    {
        return (new Rows())->parameters(Input::all())->addParameter('file_id',$dir_id)->addParameter('grid_id',$grid_name)->postAddCells()->response();
    }
    ...
    public function getActionForm($dir_id, $action, $grid_id = NULL,  $id = NULL)
    {
        $form =
            (new Form($action, $id, $grid_id))
                ->setGridId($grid_id)
                ->route(\URL::route('director-files-do-action', ['file_id' => $dir_id,'action' => $action, 'id' => $id]))
        ;
