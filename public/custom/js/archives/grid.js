;var gridArchives = function(options)
{
	$.extend( this, options);
	
	this.id = 'gridArchives';
	...
	this.dt = $('#' + this.id).DataTable({
		...
		ajax         : {
			url  : 'http://localhost/dms/be/public/archives-source',
			type : 'post',
			data : {
		...
		order        : [1, 'asc'],
		columns      : [{data : 'rec-no', orderable:false, className:'cell-right'},{data : 'name', orderable:true, className:''},{data : 'description', orderable:false, className:''},{data : 'version', orderable:false, className:''},{data : 'author', orderable:false, className:''},{data : 'path', orderable:false, className:''},{data : 'extention', orderable:false, className:''},{data : 'actions', orderable:false, className:''},],
		language     : {
		    "decimal"		    : "",
	 ...
	 */
	$(document).on('keyup change', '#gridArchives .datatable-filter-by-column', function(e){
	 ...
	 * Butonul reolad
	 */
	if($('#reload-gridArchives').length)
	{
		$('#reload-gridArchives').click( function(e){
	 ...
	 */
	if( $('#record-count-gridArchives').length )
	{
		_this.dt.on('draw.dt', function(event, settings){ 
			$('#record-count-gridArchives').html( _this.dt.page.info().recordsTotal );
		});
	}
