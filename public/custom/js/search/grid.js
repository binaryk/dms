;var gridSearch = function(options)
{
	$.extend( this, options);
	 ...
	 */
	$(document).on('keyup change', '#gridSearch .datatable-filter-by-column', function(e){
	 ...
	 * Butonul reolad
	 */
	if($('#reload-gridSearch').length)
	{
		$('#reload-gridSearch').click( function(e){
	 ...
	 */
	if( $('#record-count-gridSearch').length )
	{
		_this.dt.on('draw.dt', function(event, settings){ 
			$('#record-count-gridSearch').html( _this.dt.page.info().recordsTotal );
		});
	}
