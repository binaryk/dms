;var gridFileHistory = function(options)
{
	$.extend( this, options);
	...
	this.dt = $('#' + this.id).DataTable({
		...
		ajax         : {
			url  : 'http://localhost/dms/be/public/file-history-source/3/gridFileHistory',
			type : 'post',
			data : {
	 ...
	 */
	$(document).on('keyup change', '#gridFileHistory .datatable-filter-by-column', function(e){
	 ...
	 * Butonul reolad
	 */
	if($('#reload-gridFileHistory').length)
	{
		$('#reload-gridFileHistory').click( function(e){
	 ...
	 */
	if( $('#record-count-gridFileHistory').length )
	{
		_this.dt.on('draw.dt', function(event, settings){ 
			$('#record-count-gridFileHistory').html( _this.dt.page.info().recordsTotal );
		});
	}
